...
# from qiaosiyao_sql_model_ft.delete_table_null_attribute_one_null import delete_all_table_null_attribute
from delete_table_null_attribute import delete_all_table_null_attribute
...
'''
# houmian = 'm3e-small'
# path = os.path.join(path,houmian)
...
def tackle_data_source(request: DataSourceRequest):
    ...
    # table_info = db_tool.get_table_info(dataexample_count=10)
    logger.critical(f"***********删除信息前的table_info{table_info}***************")

    # table_info = delete_all_table_null_attribute(table_info=table_info)
    # logger.critical(f"***********10条数据中加入说有3条为null的话就删除掉这个属性：（仅有一条示例）{table_info}***************")
...
def tackle_train_data(request: TrainDataRequest):
    ...
    # query_embedding=m3e_model.encode(data)
    # logger.critical(f"*********************用于保存train_data的keep_path:{keep_path} *************")
...
def predict(request: PredictRequest):
    try:
        ...
        if not has_train_data:
            ...
            end_time = time.time()
            logger.critical(f" ************get_similar_train_data花费的时间为: {end_time - start_time} ***********")
