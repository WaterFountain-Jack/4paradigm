...
path=("/home/qiaosiyao/qiaosiyao_sql_change_embedding/AI-ModelScope/bge-base-en-v1___5")
...
class NL2SQLEmbeddingFunction(EmbeddingFunction):
    def __call__(self, input: Documents) -> Embeddings:
        embeddings = []
        for text in input:      
            cur_embedding = m3e_model.encode(text, normalize_embeddings=True).tolist()
...
def get_select_samples(json_path,total_query,single_type=True):
    ...
    if single_type:
        query_embedding=np.array(m3e_model.encode(total_query, normalize_embeddings=True))
    else:
        query_embedding=np.array(m3e_model.encode(total_query[0], normalize_embeddings=True))
        sql_embedding=np.array(m3e_model.encode(total_query[1], normalize_embeddings=True))
...
def tackle_train_data(request: TrainDataRequest):
    ...
    for index, item in enumerate(train_data_list):
       ...
       cur_query_embedding=m3e_model.encode(cur_query, normalize_embeddings=True)
