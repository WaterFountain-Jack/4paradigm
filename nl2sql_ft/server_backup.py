...
def tackle_train_data(request: TrainDataRequest):
    global is_ft
    is_ft = True
    global traindata_count
    traindata_count += 1
    if traindata_count < 9:
        return SuccessResponse(success=False)
    global has_train_data
    has_train_data = False
    #data = request.model_dump()["data"]
    # query_embedding=m3e_model.encode(query)
    #logger.critical(keep_path)
    ...
    #    embedding = m3e_model.encode(cur_query).tolist()
    #    item["embedding"] = embedding
    #    train_data_dict[cur_query] = item
...
def post_process(request: PostProcessRequest):
    ...
    if is_ft:
        ...
        return PostProcessResponse(sql=sql)
