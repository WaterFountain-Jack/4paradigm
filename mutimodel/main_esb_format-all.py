...
def create_chat_completion(request: ChatCompletionRequest):
    ...
    prefix = "Retrieve the question and its options, then output using the following template: 'Question: $question \n Options: $OptionA, $OptionB, $OptionC, $OptionD\n"
    prefix_cn= "检索问题及其选项，然后使用以下模板输出而不解决问题：问题： $question \n 选项：A:$OptionA B:$OptionB C:$OptionC D:$OptionD \n"
    prefix_english_and_cn = "If the question is in English, retrieve the question and its options, then output using the following template: 'Question: $question \\n Options: $OptionA, $OptionB, $OptionC, $OptionD\\n'. 如果问题是中文的，检索问题及其选项，然后使用以下模板输出而不解决问题：问题： $question \\n 选项：A:$OptionA B:$OptionB C:$OptionC D:$OptionD \\n"

    text_prompt, _ = model8b.chat(tokenizer8b, pixel_values, prefix_english_and_cn+text_prompt, generation_config, history=None, return_history=True)
    ...
    # sometime 26b/8b
       ...
       # 26b
        ...
        if use8bmodel == 1:
            logging.info("============== problem HIGH SCHOOL require 8b model to answer =================")
            response, history = model8b.chat(tokenizer8b, pixel_values, prompt, generation_config, history=None, return_history=True)
            if single_choice_eval(response) is None:
                prompt = text_prompt + "请从（A, B, C, D）中选择一个选项"
                response, history = model8b.chat(tokenizer8b, pixel_values, prompt, generation_config, history=history, return_history=True)
