...
def extract_fields(table_data,line_idx):
    lines = table_data.strip().split('\n')
    # 获取第二行数据
    data_line = lines[line_idx] if line_idx < len(lines) else ""
    values = data_line.split()
    return values
def extract_values(table_data,line_idx):
    lines = table_data.strip().split('\n')
    #第一行的第一数值代表了有几行示例
    num_rows = lines[0].split()[0]
...
def delete_a_table_null_attribute(create_table_sql,table_data):
    fields = extract_fields(table_data, 1)
    # 2. 读取属性对应的值，找到值为None的索引
    values = extract_fields(table_data, 2)
    # 找到值为None的属性索引
    null_indices = [i for i, value in enumerate(values) if value == 'None']

    # 3. 根据索引找到需要删除的字段
    fields_to_remove = [fields[i] for i in null_indices]

    new_create_sql = remove_fields_from_create_sql(create_table_sql, fields_to_remove)

    # 5. 重新构造表的属性和对应的值
    new_fields = [field for field in fields if field not in fields_to_remove]
    new_values = [value for i, value in enumerate(values) if i not in null_indices]
...
def delete_all_table_null_attribute(table_info):
    ...
    return table_info
def delete_all_table_null_attribut_many_example(table_info):
    table_info_list = []
    # 匹配CREATE TABLE开头的部分和以/*开头的表数据
    create_table_splits = re.split(r'CREATE TABLE', table_info, flags=re.DOTALL)
