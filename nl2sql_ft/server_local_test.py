...
#logger.critical(f"模型 url: {model_url}")
#加载m3e模型
...
collection = chroma_client.get_or_create_collection(name="nl2sql_collection", embedding_function=NL2SQLEmbeddingFunction())
...
def get_similaruty(table_recall_embedding,table_name_embedding):
    cos_similarities = [np.dot(table_recall_embedding, x) / (np.linalg.norm(table_recall_embedding) * np.linalg.norm(x)) for x in table_name_embedding]
...
def get_select_samples_v1(nl_query):
    results = collection.query(query_texts=[nl_query], n_results=3)
    most_similar_data = []
...
def tackle_data_source(request: DataSourceRequest):
    logger.critical(f"********  :{has_train_data}***************")
    ...
    logger.critical(f"db type is {db_type}")
    ...
    logger.critical(f"table names is {table_names}")
    # global table_names_embedding
    # table_names_embedding = m3e_model.encode(table_names).tolist()

    # 获取 table info
    global table_info
    table_info = db_tool.get_table_info()
    logger.critical(f"table info is {table_info}")
    return SuccessResponse(success=True)

# @app.post("/train_data")
# async def tackle_train_data(request: TrainDataRequest):
#     logger.critical("*********成功获得了train_data***********")
#     print("print成功获得了train_data Received request data:")  # 打印接收到的数据
#     return SuccessResponse(success=True)

@app.post("/train_data")
def tackle_train_data(request: TrainDataRequest):
    print("**********pirnt 成功获得了traindata***************")
    logger.critical("******logger***成功获得了train_data***********")
    global has_train_data
    has_train_data = True


    # data = request.data
    # with open(keep_path, 'w') as json_file:
...
def predict(request: PredictRequest):
    try:
        ...
        if not has_train_data:
            ...
            logger.critical(f"###########进入微调阶段############")
            
            
            
            #获取训练数据
            
            logger.critical(f"###########选择取样ing############")    
            samples=get_select_samples(keep_path,nl_query)
            logger.critical(f"###########提供的数据取样结果############") 
            #logger.critical(samples)
            
            #创建table的prompt：
            table_prompt=build_ft_table_prompt(query=nl_query,table_info=table_info,db_type=db_type,examples=samples)
            logger.critical("###########table_prompt############")
            #logger.critical(table_prompt)
            
            # selected_table_names=table_infer(prompt=table_prompt,infer_way="vllm",model="public/deepseek-coder-33b-instruct@main",
            #                                     base_url="table_model_url",temperature=0.25,stop_words=["<|EOT|>","SQLQuery:"])
            
            # table_selected_info=get_table_info_for_tables(db_tool, selected_table_names)
            
            #让其自己推理产生解释
            cot_prompt=build_summary_prompt(db_type=db_type,database_info=table_selected_info,examples=samples)
            
            sys,user=split_prompt(cot_prompt)
            explanation=infer(sys_prompt=sys,user_prompt=user,infer_way="vllm",type="解释的推理",
                              base_url="table_model_url",model="public/deepseek-coder-33b-instruct@main",stop_words=["<|EOT|>","Result:"])
            index = explanation.find('Explanation:')
            # 分割字符串
            examples_info = explanation[index:]
            prompt=build_cot_ft_sql_prompt(query=nl_query,db_type=db_type,examples=samples,examples_info=examples_info,table_selected_info=table_selected_info)
            
            #prompt=build_ft_sql_prompt(query=nl_query,db_type=db_type,examples=samples,table_selected_info=table_selected_info)
...
if __name__ == "__main__":
    port = 18089
    uvicorn.run("server_local_test:app", host="0.0.0.0", port=port)
