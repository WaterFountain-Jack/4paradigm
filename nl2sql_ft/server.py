...
def print_available_cuda_devices():
    if torch.cuda.is_available():
        ...
        logger.critical(f"Number of available CUDA devices: {num_devices}")
        for device_id in range(num_devices):
            device_name = torch.cuda.get_device_name(device_id)
            logger.critical(f"CUDA Device {device_id}: {device_name}")
    else:
        logger.critical("CUDA is not available")
        
def print_cuda_memory():
    result = subprocess.run(['nvidia-smi', '--query-gpu=memory.free,memory.total,memory.used', '--format=csv,noheader,nounits'], capture_output=True, text=True)

    # 解析输出
    lines = result.stdout.strip().split('\n')
    for i, line in enumerate(lines):
        free, total, used = map(int, line.split(','))
        logger.critical(f"GPU {i}:")
        logger.critical(f"  Total memory: {total} MB")
        logger.critical(f"  Free memory: {free} MB")
        logger.critical(f"  Used memory: {used} MB")
app = FastAPI()
...
#用于测试不同的模型链接
logger.critical("***********加载Embedding模型ing************")
# path=("/app/bce-embedding-base_v1")
path=("/app/embedding_models")

logger.critical(f"***********加载Embedding的路径为{path}  并且 normalize_embeddings=True************")
# print_cuda_memory()
# m3e_model=SentenceTransformer(path, device='cpu')
m3e_model=SentenceTransformer(path)

print_available_cuda_devices()


logger.critical(f"***********m3e模型的参数在{next(m3e_model.parameters()).device}************")
# m3e_model.to('cpu')
logger.critical("***********加载Embedding模型成功************")
...
def get_similar_train_hand(query,train_data_list = train_data_list):
    
    
    logger.critical(f"************type(train_data_list[0]['cur_query_embedding']):{type(train_data_list[0]['cur_query_embedding'])}*******************")
    query_embedding=np.array(m3e_model.encode(query))
    
    
    time0 = time.time()
    cos_similarities = [np.dot(query_embedding, x['cur_query_embedding']) / (np.linalg.norm(query_embedding) * np.linalg.norm(x['cur_query_embedding'])) for x in train_data_list]
    time1 = time.time()
    logger.critical(f"************计算相似度的的时间为{time1 - time0}*******************")
    k = 3
    indices = np.argpartition(-np.array(cos_similarities), k)[:k]
    time2 = time.time()
    logger.critical(f"************获取前k个最相似的时间为{time2 - time1}*******************")
    
    return [train_data_list[idx] for idx in indices]
...
async def tackle_train_data(request:Request):
    ...
    try:
        ...
        logger.critical("*********************成功接收到到了data_dict*************")
    ...
    train_data_list = data_dict["data"]
...
def predict(request: PredictRequest):
    ...
    try:
        ...
        if not has_train_data:
            ...
            global train_data_list
            global train_data_collection
            logger.critical(f"len(train_data_list):{len(train_data_list)}    ")
            start_time = time.time()
            example = get_similar_train_hand(nl_query,train_data_list)
            end_time = time.time()
            logger.critical(f" ************get_similar_train_hand花费的时间为: {end_time - start_time} ***********")
            
            prompt = build_prompt_train_data(nl_query, table_selected_info,example)
            end_time1 = time.time()
            logger.critical(f" ************build_prompt_train_data花费的时间为: {end_time1 - end_time} ***********")
        
        
        start_time = time.time()
        response = openai_infer(prompt=prompt, stop=request.stop)
        end_time = time.time()
        logger.critical(f" ************openai_infer花费的时间为: {end_time - start_time} ***********")
        logger.critical(f" ************predict花费的时间为: {end_time - predict_time_begin} ***********")
