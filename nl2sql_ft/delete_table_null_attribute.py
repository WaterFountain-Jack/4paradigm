...
def extract_fileds(table_data, line_idx):
    lines = table_data.strip().split('\n')
    # 获取第二行数据
    data_line = lines[line_idx] if line_idx < len(lines) else ""
    values = data_line.split()
    return values


def delete_null_values(table_data, yuzhi):


    #返回大于阈值次数null的list
    lines = table_data.strip().split('\n')
    #lane0
    table_msg_list = lines[0].split()
    table_msg_list[0] = '1'

    #第一行的指是line
    fields = lines[1].split()
    #用一个list统计None出现的次数
    null_list = [0] * len(fields)
    #统计第二行到最后一行
    for i in range(2,len(lines)):
        values = lines[i].split()
        for j in range(len(values)):
            if (values[j] == "None"):
                null_list[j] = null_list[j] + 1

    idx_shanchu = [idx for idx,val in enumerate(null_list) if val > yuzhi ]
    idx_shengyu = [idx for idx,val in enumerate(null_list) if val <= yuzhi ]

    fields_shanchu_list = [fields[idx] for idx in idx_shanchu]
    fields_shengyu_list = [fields[idx] for idx in idx_shengyu]

    #选择第一个数据作为base
    a_row_value_list = lines[2].split()
    a_row_value_shengyu_list = [a_row_value_list[idx] for idx in idx_shengyu]
    ...
    */
    ...
    new_table_data_str = f'''/*
{' '.join(table_msg_list)}
{' '.join(fields_shengyu_list)}
{' '.join(a_row_value_shengyu_list)}
*/
'''
    #-1代表需要删除的 1代表不需要删除
    return new_table_data_str,fields_shanchu_list




# 4. 删除CREATE TABLE语句中对应属性的部分
def remove_fields_from_create_sql(create_table_sql, fields_to_remove):
    # 拆分CREATE TABLE语句的各个字段部分
    create_lines = create_table_sql.splitlines()
    new_create_lines = []

    for line in create_lines:
        # 检查每一行是否包含要删除的字段
        if not any(field in line for field in fields_to_remove):
            new_create_lines.append(line)

    # 重新构建CREATE TABLE语句
    new_create_sql = "\n".join(new_create_lines)
    return new_create_sql



def delete_a_table_null_attribute(create_table_sql,table_data):


    yuzhi = 1
    new_table_data_str,fields_to_remove = delete_null_values(table_data,yuzhi)

    new_create_sql = remove_fields_from_create_sql(create_table_sql, fields_to_remove)


    a_table_info = new_create_sql + '\n\n'+ new_table_data_str +  '\n\n'
    return a_table_info


def delete_all_table_null_attribute(table_info):
    table_info_list = []
    # 匹配CREATE TABLE开头的部分和以/*开头的表数据
    create_table_splits = re.split(r'CREATE TABLE', table_info, flags=re.DOTALL)

    # 遍历每个CREATE TABLE语句和表数据对
    for i in range(1, len(create_table_splits)):
        a_table_info_dict = {

        }
        # 重新加上CREATE TABLE，因为之前分割时去掉了
        create_table_split = 'CREATE TABLE' + create_table_splits[i]

        # 分割CREATE TABLE语句和表数据部分
        table_sql, table_data = re.split(r'/\*', create_table_split, 1)

        a_table_info_dict["split_data_list"] = table_sql.strip()
        a_table_info_dict["table_data_list"] = table_data.strip().rstrip('*/').strip()

        table_info_list.append(a_table_info_dict)
    table_info = ""
    for i in table_info_list:
        table_info = table_info + delete_a_table_null_attribute(i['split_data_list'],i['table_data_list'])
...
)COMMENT='资产负债表' DEFAULT CHARSET=utf8mb4 ENGINE=InnoDB COLLATE utf8mb4_unicode_ci
...
)COMMENT='现金流量表' DEFAULT CHARSET=utf8mb4 ENGINE=InnoDB COLLATE utf8mb4_unicode_ci
...
'''
a = delete_all_table_null_attribute(data_string)
print(a)
