...
def split_prompt1(prompt,split_words=None):
    # 找到 "today". 的位置
    if split_words is None:
        index = prompt.find('Some SQL examples are provided based on similar problems')
    else:
        index = prompt.find(split_words)
    # 分割字符串
    part1 = prompt[:index]
...
#         prompt  = prompt + f"""
# {table["talble_message"]}
# /*
# f{table["table_example"]}
# */
...
"""
    for data in example[0:num_example]:
...
def tackle_data_source(request: DataSourceRequest):
    ...
    logger.critical(f"*************设定查询结果行的数量为1***************")
    #查询结果行的数量为1，之后查询的结果只需要一个
    db_tool._sample_rows_in_table_info=5
...
def predict(request: PredictRequest):
    try:
        ...
        if not has_train_data:
            ...
            table_selected_info = db_tool.get_table_info()
            global train_data_list
            prompt = build_prompt_train_data(nl_query, table_selected_info,train_data_list)
