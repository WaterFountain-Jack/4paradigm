...
class SuccessResponse(BaseModel):
    success: bool
def get_similar_train_data_test(query,train_data_collection = train_data_collection,train_data_list = train_data_list):
    # query_embedding=np.array(m3e_model.encode(query))

    logger.critical(f"***************开始运行train_data_collection.query****************")
    results = train_data_collection.query(
    query_texts=[query],
    n_results=3  # 获取最接近的k个结果
    )
    logger.critical(f"***************查询到的结果为{results}****************")
...
def get_similar_train_data(query,train_data_collection = train_data_collection,train_data_list = train_data_list):
    ...
    logger.critical(f"*********查询到与当前query最近的索引为{results[0]['ids']}****************")
    return train_data_list[results[0]['ids']]
# results:    [
#     {
...
def tackle_train_data(request: TrainDataRequest):
    ...
    logger.critical(f"*********************用于训练的train_data:{train_data_list} *************")
...
def predict(request: PredictRequest):
    try:
        ...
        if not has_train_data:
            ...
            example  = get_similar_train_data(nl_query)
